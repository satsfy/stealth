...
use stealth_core::scanner::{RpcAuth, RpcConfig, ScanTarget, UtxoInput};
...
fn run_scan(args: &[String]) -> Result<bool, String> {
    ...
    let config = opts.rpc_config()?;
    let target = opts.scan_target()?;

    let report = stealth_core::scanner::scan(&config, target).map_err(|e| e.to_string())?;
...
impl ScanOpts {
    fn rpc_config(&self) -> Result<RpcConfig, String> {
        let url = self
            .rpc_url
            .clone()
        ...
        let auth = match (
            ...
            (_, _, Some(cookie)) => RpcAuth::CookieFile(cookie),
            _ => RpcAuth::None,
        };

        Ok(RpcConfig { url, auth })
    }
...
fn print_text_report(report: &stealth_core::Report) {
    println!(
