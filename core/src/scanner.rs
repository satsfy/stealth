//! Thin compatibility layer around the legacy direct-RPC scan path.
//!
//! **The canonical execution path is
//! [`AnalysisEngine`](crate::engine::AnalysisEngine) +
//! [`BlockchainGateway`](crate::gateway::BlockchainGateway).**
//!
//! This module is retained so that `core/tests/integration.rs` (which
//! builds a [`TxGraph`] from a live `corepc-client`) keeps compiling.
//! New code should use the engine/gateway path instead.
...
// Re-export the canonical types so that existing `use
// stealth_core::scanner::{ScanTarget, UtxoInput}` paths keep working.
pub use crate::engine::{ScanTarget, UtxoInput};
...
///
/// **Prefer [`AnalysisEngine::analyze`](crate::engine::AnalysisEngine::analyze)
/// for new code.**
