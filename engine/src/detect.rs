...
impl TxGraph {
    ...
    fn detect_dust(&self, thresholds: &DetectorThresholds, findings: &mut Vec<Finding>) {
        let dust_sats = thresholds.dust_sats;
        let strict_dust = thresholds.strict_dust_sats;
        ...
        let txids: Vec<Txid> = self.our_txids.iter().copied().collect();
        let current_keys: HashSet<(Txid, String)> = self.utxos
            .iter()
            .map(|u| (u.txid, u.address.clone()))
            .collect();
        ...
        for txid in &txids {
            ...
            for out in &outputs {
                ...
                if sats <= dust_sats && self.is_ours(&out.address) {
                    let key = (*txid, out.address.clone());
    ...
    fn detect_dust_spending(&self, thresholds: &DetectorThresholds, findings: &mut Vec<Finding>) {
        let dust_sats = thresholds.dust_sats;
        let normal_min = thresholds.normal_input_min_sats;
    ...
    fn detect_consolidation_origin(&self, thresholds: &DetectorThresholds, findings: &mut Vec<Finding>) {
    ...
    fn detect_lookback_depth(&self, thresholds: &DetectorThresholds, findings: &mut Vec<Finding>, warnings: &mut Vec<Finding>) {
    ...
    fn detect_exchange_origin(
        ...
        for txid in &txids {
            ...
            let total_input_sats: u64 = input_addrs.iter().map(|ia| ia.value_sats).sum();
            let mut output_values: Vec<u64> = tx
                .vout
                .iter()
                .map(|o| btc_to_sats(o.value_btc))
                .collect();
            output_values.sort_unstable();
            if !output_values.is_empty() {
                let median = output_values[output_values.len() / 2];
                if median > 0 && total_input_sats > 10 * median {
                    signals.push(format!(
                        "Input/median-output ratio: {}x (exchange-like fan-out)",
                        total_input_sats / median
                    ));
                }
            }
