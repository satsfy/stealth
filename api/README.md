...
# and uses credentials from ~/.bitcoin/bitcoin.conf or local cookie files.
...
| `STEALTH_RPC_USER` | RPC username (otherwise read from `~/.bitcoin/bitcoin.conf` when available) |
| `STEALTH_RPC_PASS` | RPC password (otherwise read from `~/.bitcoin/bitcoin.conf` when available) |
