...
struct ApiServer {
    ...
    gateway: Option<Arc<dyn stealth_core::gateway::BlockchainGateway + Send + Sync>>,
}

impl ApiServer {
    async fn spawn(gateway: BitcoinCoreRpc) -> Self {
        let gateway: Arc<dyn stealth_core::gateway::BlockchainGateway + Send + Sync> =
            Arc::new(gateway);
        ...
        let server =
            axum::serve(listener, stealth_api::app_with_gateway(Some(gateway.clone())))
